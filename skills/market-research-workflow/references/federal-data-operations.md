...
Agent configurations pin `sam-gov-mcp==1.0.12` and `usaspending-gov-mcp==1.0.4` with an explicit three-second pacing safeguard. USASpending uses the `acquisition-agent` tool profile in packaged agents while the standalone server retains its complete tool catalog.
