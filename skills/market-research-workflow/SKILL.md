...
5. Run `scripts/build_market_research_report.py <record.json> <output.docx>`.
6. Run `scripts/validate_market_research_report.py <output.docx> --record <record.json>`.
7. Independently recompute numeric tables from the record and compare them with the document.
8. Open/save through LibreOffice and convert to PDF.
9. Extract text and citations.
